...
1.  What is the *population* in this setting? Please be precise. \*updated

Answer: The population in this instance refers to the air quality Lahore from November 2018 to November 2019.

2.  What are the *samples* in this setting? Please be precise. \*updated

Answer: The samples are the areas where the monitors are installed and collecting data for the government and the crowdsourced data.

3.  These samples were not randomly collected from across locations in Lahore. Given the sampling approaches described above, discuss possible biases that may enter when we use these samples to construct estimates of population parameters. \*updated

Answer: Possible biases include the "neighbor mentality"; for example, one might be more likely to include a monitor on their property if their neighbor does it and vice versa. This introduces the possibility for certain behaviors to be present in one population (the population who accepted having monitors on their property) compared to the other (the population who didn't have monitors on their property). Therefore, when estimates on population parameters are made, they may only represent the population who accepted having monitors on their property. Additionally, it is suspected that the government purposefully set monitors in areas where the air quality is better which also sways the air quality data in a "less bad" direction.
...
3.  Are the differences in mean pollution as expected, given what we know about the sampling strategies? \*updated

Answer: Yes, given that the government has been suspected to report values lower than actuality, it makes sense that the govt mean, max, and min would be significantly lower than the crowdsourced mean, max, and min. Additionally, it is important to note that the amount of crowdsourced data (number of rows) is higher in quantity than the govt data. This also makes sense when considering the government's under-reporting suspicions. Also the biases present in the crowdsourced data, such as education and being potentially wealthier and therefore living in areas of lower air pollution may not hold up as well with the means presented. However, more information (such as actual values) are needed to assess accuracy of the crowdsourced data.
...
First, generate a *random sample* of size $n=1000$ air pollution records by (i) pooling observations across the government and the crowd-sourced data; and (ii) drawing observations at random from this pooled sample. \* update
...
# Dimensions of stratefied sample
dim(strat_sample)
